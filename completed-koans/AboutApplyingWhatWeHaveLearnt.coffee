...
describe 'About Applying What We Have Learnt', ->
  ...
  it 'should find needle in a haystack (functional)', ->
    ...
    findNeedle = (ops) ->
      hasInvalidOperation = false
      hasInvalidOperation = true for direct, dist in operations when direct is 'FWD' && dist > 100
    findNeedle(operations).should.equal 
  ...
  it 'should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)', ->
    ...
    total = 0
    sum = (a) -> total += a
    sum i for i in [1..1000] when i % 3 is 0 || i % 5 is 0
    total.should.equal 234168
  ...
  it 'should find the sum of all the even valued terms in the fibonacci sequence which do not exceed four million (functional)', ->
    ...
    i = 3
    even = (n) -> n % 2 == 0
    currentFib = (i, j) -> fib[i] + fib[j]
    sum = (total, x) -> total + x
    i += 1 while (fib[i] = currentFib i - 1, i - 2) < 4000000

    ((x for x in fib when even(x)).reduce sum).should.equal 4613732
